...
function mouseoverNode(d) {
    if(focus == null) {
        putInfo(d.id, d3.event.pageX, d3.event.pageY);
    } else if(focus.id != d.id) {
        putInfo(d.id, d3.event.pageX, d3.event.pageY);
    }
}
...
function mouseoverLine(d) {
    if(focus != null) {
        if(d.source.id == focus.id) {
            putInfo(d.target.id, d3.event.pageX, d3.event.pageY);
        } else if(d.target.id == focus.id) {
            putInfo(d.source.id, d3.event.pageX, d3.event.pageY);
        }
    }
}
...
function putInfo(text, x, y) {
    infobox.transition()
        .duration(50)
        .style("opacity", 1.0)
        .style("z-index", 10);
    infobox.html("<span>" + text + "</span>");
